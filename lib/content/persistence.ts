...
const DATA_FILE = path.join(DATA_DIR, "site-sections.json");
const BLOB_PATH = "wedding/site-sections.json";
...
export async function readSiteSectionsRaw(): Promise<string> {
...
export async function writeSiteSectionsRaw(json: string): Promise<void> {
  if (isBlobStorageEnabled()) {
